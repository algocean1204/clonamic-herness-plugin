...
class CallTests(unittest.TestCase):
    ...
    def test_success_is_json_and_redacted(self) -> None:
        ...
        self.assertEqual(
            ...
            "argv=['exec', '--ephemeral', '--sandbox', 'read-only', '--model', 'test-model', '-']",
        )
        self.assertIn("prompt='hello'", result["output"])
        self.assertNotIn("hello", result["output"].splitlines()[0])
    ...
    def test_windows_termination_uses_kill_on_close_job(self) -> None:
        spec = importlib.util.spec_from_file_location("clonamic_gpt_call", CALL)
    ...
    def test_skill_forbids_self_hosting(self) -> None:
        ...
        self.assertIn("Never call this Codex wrapper from Codex itself", skill)
