#!/usr/bin/env python3
"""Run one bounded Codex CLI request and emit one JSON result."""
...
def start_capture(proc: subprocess.Popen[bytes]) -> tuple[list[threading.Thread], BoundedCapture, BoundedCapture]:
...
def terminate(proc: subprocess.Popen[bytes], *, platform: str | None = None) -> None:
...
def command(executable: str, cli_args: list[str], prompt: str) -> list[str]:
    return [executable, "exec", "--ephemeral", "--sandbox", "read-only", *cli_args, prompt]
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        proc = subprocess.Popen(
            command(executable, cli_args, prompt),
    ...
    threads, stdout_capture, stderr_capture = start_capture(proc)
    try:
