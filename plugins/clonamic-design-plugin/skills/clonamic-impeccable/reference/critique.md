...
2. **Compute the slug**:
   ```bash
   node .claude/skills/clonamic-impeccable/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
   Keep it. If the command exits non-zero, skip persistence and trend for this run, but continue the critique.
3. **Read `.clonamic-impeccable/critique/ignore.md`** if it exists. Drop matching findings silently; it is the only prior-run input critique consumes.
...
Evaluate:
- **AI slop**: Would someone believe "AI made this" immediately? Check all DON'T guidance from the parent Clonamic Impeccable skill.
...
CLI scan:
```bash
node .claude/skills/clonamic-impeccable/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node .claude/skills/clonamic-impeccable/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `clonamic-impeccable` console messages, then stop the live server.
...
- **Suggested command**: Which command could address this (from: /clonamic-impeccable adapt, /clonamic-impeccable animate, /clonamic-impeccable audit, /clonamic-impeccable bolder, /clonamic-impeccable clarify, /clonamic-impeccable colorize, /clonamic-impeccable critique, /clonamic-impeccable delight, /clonamic-impeccable distill, /clonamic-impeccable document, /clonamic-impeccable harden, /clonamic-impeccable layout, /clonamic-impeccable onboard, /clonamic-impeccable optimize, /clonamic-impeccable overdrive, /clonamic-impeccable polish, /clonamic-impeccable quieter, /clonamic-impeccable shape, /clonamic-impeccable typeset)

#### Persona Red Flags
> *Consult the [Personas reference](#persona-based-design-testing) below.*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `CLAUDE.md` contains a `## Design Context` section from `clonamic-impeccable init`, also generate 1-2 project-specific personas from the audience/brand info.
...
Once the report above is finalized, write it to `.clonamic-impeccable/critique/` so the user can refer back, and so `/clonamic-impeccable polish` can pick up the priority issues without a copy-paste.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node .claude/skills/clonamic-impeccable/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node .claude/skills/clonamic-impeccable/scripts/critique-storage.mjs trend <slug> 5
   ```
...
5. **Append a single line to the user-visible output**, after the report and before the questions:
   ...
   > Wrote `.clonamic-impeccable/critique/<filename>`.
...
- Only recommend commands from: /clonamic-impeccable adapt, /clonamic-impeccable animate, /clonamic-impeccable audit, /clonamic-impeccable bolder, /clonamic-impeccable clarify, /clonamic-impeccable colorize, /clonamic-impeccable critique, /clonamic-impeccable delight, /clonamic-impeccable distill, /clonamic-impeccable document, /clonamic-impeccable harden, /clonamic-impeccable layout, /clonamic-impeccable onboard, /clonamic-impeccable optimize, /clonamic-impeccable overdrive, /clonamic-impeccable polish, /clonamic-impeccable quieter, /clonamic-impeccable shape, /clonamic-impeccable typeset
...
- End with `/clonamic-impeccable polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `/clonamic-impeccable critique` after fixes to see your score improve.
...
If `CLAUDE.md` contains a `## Design Context` section (generated by `clonamic-impeccable init`), derive 1–2 additional personas from the audience and brand information:
