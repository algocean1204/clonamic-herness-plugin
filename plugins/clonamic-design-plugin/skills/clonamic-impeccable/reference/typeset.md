...
When the type carries the hierarchy on its own, hand off to `/clonamic-impeccable polish` for the final pass.
...
Treat host-supplied parameters as task-local and do not persist them implicitly.
