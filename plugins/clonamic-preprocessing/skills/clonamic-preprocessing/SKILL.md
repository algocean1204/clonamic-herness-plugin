...
- `normalize_text(text)` — normalize text without changing meaning.
...
The queue stores normalized text, priority, sequence, state, claim token, attempts, and result. Mutations use bounded locking and atomic replacement. Reclaimed stale work invalidates its old token.
