...
def render_pngs(pptx: Path, out_dir: Path) -> list[Path]:
    ...
    with tempfile.TemporaryDirectory() as td:
        td_path = Path(td)
        proc = subprocess.run(
            [soffice, "--headless", "--norestore", "--convert-to", "pdf", "--outdir", str(td_path), str(pptx)],
            capture_output=True,
            text=True,
        )
        ...
        if shutil.which("pdftoppm"):
            ...
            subprocess.run(
                [soffice, "--headless", "--norestore", "--convert-to", "png", "--outdir", str(out_dir), str(pdf)],
                capture_output=True,
            )
...
def qa_visual(pptx: Path, deck: dict, png_dir: Path) -> list[dict]:
    ...
    if not pngs:
        issues.append(issue("info", "VIS000", "visual render skipped (no soffice/pdftoppm)"))
        return issues
