...
def native_manifest_path(manifest_path, platform):
    ...
    return package / NAMESPACE / platform / "plugin.json"


def descriptor_native_path(manifest_path, platform):
    parent = PurePosixPath(manifest_path).parent
    if parent.as_posix() == ".":
        parts = [platform, "plugin.json"]
    else:
        parts = ["..", *parent.parts, NAMESPACE, platform, "plugin.json"]
...
def main():
    ...
    if args.check:
        ...
        return 0
    stale = managed_output_paths() - set(outputs)
